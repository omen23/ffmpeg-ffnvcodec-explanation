...
`GpuVideoDecoder` is deprecated now! *(altough it seems some old systems still need it – I think it has to be patched into the source, you have to start `chromium` with `--disable-mojovideodecoder` or `--enable-gpuvideodecoder` – I am not sure — what **is sure** `MojoVideoDecoder` has a much better performance than `GpuVideoDecoder` on more recent rigs!)*
